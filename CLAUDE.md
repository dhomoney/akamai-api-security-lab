...
**CRITICAL — baseline must be fully trained before attacks will register.** Noname's behavioral detections (BOLA, credential stuffing anomalies, rate abuse, etc.) are purely comparative — the engine measures attack traffic against a learned normal pattern. Endpoints still in the **Learning** state in the Noname UI will produce zero alerts regardless of how many attack requests are sent. Check Settings → Traffic Sources → source detail: every API endpoint that matters for detection must show **Trained** / **Active** status (not green/amber "Learning") before launching `make traffic-owasp-aws`. In practice this means running `make traffic-aws` for **at least 60 minutes** (longer if the engine shows many endpoints still learning). Do not start the attack phase early — BOLA and other behavioral categories will silently produce no detections until training completes.
